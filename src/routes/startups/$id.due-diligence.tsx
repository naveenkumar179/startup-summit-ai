        ...
        <>
          ...
          <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
            <div className="rounded-xl bg-accent/40 p-3 text-center">
              <div className="text-lg font-bold text-foreground">{a.investmentReadinessScore}/100</div>
              <div className="text-xs text-muted-foreground">Investment Score</div>
            </div>
            <div className="rounded-xl bg-accent/40 p-3 text-center">
              <div className="text-lg font-bold text-success">
                {a.investmentReadinessScore >= 70 ? "High" : a.investmentReadinessScore >= 45 ? "Medium" : "Low"}
              </div>
              <div className="text-xs text-muted-foreground">Growth Potential</div>
            </div>
            <div className="rounded-xl bg-accent/40 p-3 text-center">
              <div className="text-lg font-bold text-amber-600 dark:text-amber-400">
                {a.riskAnalysis.length >= 4 ? "High" : a.riskAnalysis.length >= 2 ? "Low-Medium" : "Low"}
              </div>
    ...
    },
    {
      ...
      render: (a) => (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            <h4 className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-success">Strengths</h4>
            <BulletList items={a.swot.strengths} />
          </div>
          <div>
            <h4 className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-destructive">Weaknesses</h4>
            <BulletList items={a.swot.weaknesses} />
          </div>
          <div>
            <h4 className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-primary">Opportunities</h4>
      ...
      <div className="mx-auto max-w-5xl space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground">AI-generated comprehensive report for {data.startup.name}</p>
          </div>
...
function sectionToPlainText(id: string, a: DetailedAnalysis, s: Startup): string[] {
  switch (id) {
    ...
    case "conclusion":
      return [`Based on an investment readiness score of ${a.investmentReadinessScore}/100, ${a.recommendation}`];
    default:
      return [];
  }
}
